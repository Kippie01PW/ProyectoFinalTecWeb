...
if (!isset($_POST['maestro_id']) || !isset($_POST['accion']) || empty($_POST['maestro_id']) || empty($_POST['accion'])) {
...
error_log("Debug - Maestro ID: " . $maestro_id);

if (!$maestro_id) {
    echo json_encode(['success' => false, 'message' => 'ID de maestro inválido']);
    exit;
}
...
try {
    ...
    switch ($accion) {
        case 'correo':
            ...
            $stmt_check = $conexion->prepare("SELECT id FROM usuarios WHERE email = :email AND id != :maestro_id");
            $stmt_check->bindParam(':email', $nuevo_correo);
            $stmt_check->bindParam(':maestro_id', $maestro_id, PDO::PARAM_INT);
            $stmt_check->execute();
            ...
            $stmt_user = $conexion->prepare("SELECT id FROM usuarios WHERE id = :maestro_id");
            $stmt_user->bindParam(':maestro_id', $maestro_id, PDO::PARAM_INT);
            $stmt_user->execute();
            ...
            $stmt_update = $conexion->prepare("UPDATE usuarios SET email = :email WHERE id = :maestro_id");
            $stmt_update->bindParam(':email', $nuevo_correo);
            $stmt_update->bindParam(':maestro_id', $maestro_id, PDO::PARAM_INT);
        ...
        case 'contrasena':
            ...
            error_log("Debug - Verificando usuario ID: " . $maestro_id);
            ...
            $stmt_update = $conexion->prepare("UPDATE usuarios SET password_hash = :password WHERE id = :maestro_id");
            ...
            if ($stmt_update->execute()) {
                ...
                error_log("Contraseña actualizada para usuario ID: $maestro_id en " . date('Y-m-d H:i:s'));
            } else {
    ...
    error_log("Error PDO en actualizar_maestro.php: " . $e->getMessage());
    $response = [
        'success' => false, 
        'message' => 'Error PDO: ' . $e->getMessage()
    ];
} catch (Exception $e) {
    error_log("Error general en actualizar_maestro.php: " . $e->getMessage());
    $response = [
        'success' => false, 
